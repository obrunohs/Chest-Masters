...
window.registerAccount = async function () {
  ...
  try {
    ...
    // Importante: força o Firebase Auth a atualizar o token
    // antes de gravar o perfil do jogador no Firestore.
    await cred.user.getIdToken(true);

    await ensurePlayerDoc(cred.user, name);
    await sendEmailVerification(cred.user);
    await signOut(auth);
    window.showScreen("loginScreen");
    alert("Conta criada. Confirme o email antes de entrar.");
  } catch (err) {
